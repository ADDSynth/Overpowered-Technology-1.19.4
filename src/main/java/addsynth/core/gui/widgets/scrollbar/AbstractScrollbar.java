...
public abstract class AbstractScrollbar<E, L extends AbstractListEntry<E>> extends AbstractWidget {
  ...
  private final void recalculateScrollbar(){
    try{
      ...
      max_position_y = getY() + height - scrollbar_height;
      index_positions = MathUtility.getPositions(getY(), max_position_y, max_positions);
  ...
  public void renderWidget(PoseStack matrix, int p_render_1_, int p_render_2_, float p_render_3_){
    ...
    WidgetUtil.verticalSplitRender(matrix,
      new Dimensions(x, getY(), scrollbar_gui_width, height, scrollbar_gui_width, max_scrollbar_height),
  ...
  protected final void onDrag(double gui_x, double gui_y, double screen_x, double screen_y){
    ...
    position_y = Mth.clamp(center_y - scrollbar_half_height, getY(), max_position_y);
