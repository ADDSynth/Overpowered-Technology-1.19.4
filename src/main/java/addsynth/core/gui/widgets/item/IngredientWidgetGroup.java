...
public final class IngredientWidgetGroup {
  ...
  */
  ...
  public final void drawIngredient(final ItemRenderer itemRenderer, PoseStack matrix, int index, int x, int y){
    ingredient[index].draw(itemRenderer, matrix, x, y);
  }
  
  // public final void draw(int[] x, int[] y){
  // }
  
  public final void draw(final ItemRenderer itemRenderer, PoseStack matrix, int guiLeft, int[] x, int guiTop, int[] y){
    for(i = 0; i < length; i++){
      ingredient[i].draw(itemRenderer, matrix, guiLeft + x[i], guiTop + y[i]);
    }
  }
