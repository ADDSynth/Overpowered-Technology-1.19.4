...
public abstract class Checkbox extends AbstractButton {
  ...
  public final void renderWidget(PoseStack matrix, final int mouseX, final int mouseY, final float partial_ticks){
    ...
    blit(matrix, getX(), getY(), gui_size, gui_size, checked ? texture_x : texture_x + texture_height, texture_y, texture_width, texture_height, 256, 256);
    font.draw(matrix, getMessage(), getX() + 16, getY() + 2, GuiUtil.text_color);
  }
