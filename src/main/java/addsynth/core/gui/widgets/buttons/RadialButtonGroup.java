...
public final class RadialButtonGroup extends AbstractButton {
  ...
  public void renderWidget(PoseStack matrix, int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_){
    ...
    for(i = 0; i < buttons; i++){
      blit(matrix, getX(), getY() + (line_height*i), radial_gui_size, radial_gui_size, radial_texture_x, i == option_selected ? radial_selected_texture_y : radial_texture_y, radial_texture_size, radial_texture_size, 256, 256);
    }
    for(i = 0; i < buttons; i++){
      font.draw(matrix, options[i], getX() + 16, getY() + (line_height*i) + 2, GuiUtil.text_color);
    }
  }
